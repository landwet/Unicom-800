 ...
 initTasksQueue: async () => {
    ...
    if (!fs.existsSync(scheduler.taskFile)) {
      ...
      if (taskJson.today !== today) {
        ...
        console.log("📑 嫖资配置更新完毕 等待5秒再继续");
        // eslint-disable-next-line no-unused-vars
        await new Promise((resolve, reject) => setTimeout(resolve, 5 * 1000));
      }

      if (taskJson.queues.length !== Object.keys(tasks).length) {
        console.log("📑 数量已变更，重新生成嫖资配置");
        let queues = await scheduler.buildQueues();
        fs.writeFileSync(
          scheduler.taskFile,
          JSON.stringify({
    ...
    genFileName(command) {
        ...
        console.info('🚨获得少妇房间🚨', maskFile, '当前日期', scheduler.today)
    },
    loadTasksQueue: async (selectedTasks) => {
        ...
        console.info('计算可嫖少妇', '总少妇数', queues.length, '已嫖少妇数', queues.filter(t => t.taskState === 1).length, '错误少妇数', queues.filter(t => t.taskState === 2).length, '指定少妇数', selectedTasks.length, '预计可嫖少妇数', will_tasks.length)
        return {
            taskJson,
            queues,
            will_tasks
    ...
    hasWillTask: async (command, params) => {
        ...
        if (scheduler.isTryRun) {
            console.info('⚠️温馨提示你已进入少妇房间☢快速进入活塞运动战斗⚠️')
            await new Promise((resolve) => setTimeout(resolve, 3000))
        }
        process.env['taskKey'] = [command, scheduler.taskKey].join('_')
        process.env['command'] = command
        console.info('🎴获得嫖客识别码🃏', scheduler.taskKey.replaceWithMask(2, 3), '㊙️')
    ...
    execTask: async (command) => {
        console.info('🚀已驶入少妇房间,自动战斗1⃣小时活塞运动☢')
        ...
        if (selectedTasks.length) {
            console.info('将只执行选择的少妇', selectedTasks.join(','))
        }
